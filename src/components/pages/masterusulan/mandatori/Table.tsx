...
const Table = () => {
    ...
    const [Mandatori, setMandatori] = useState<Usulan[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchMandatori = async() => {
            setLoading(true)
            try{
                const response = await fetch(`${API_URL}/usulan_mandatori/findall`, {
                    headers: {
                        Authorization: `${token}`,
                ...
                const data = result.usulan_mandatori;
                if(data == null){
                    setDataNull(true);
                    setMandatori([]);
    ...
    const hapusMusrenbang = async(id: any) => {
        ...
        try{
            ...
            setMandatori(Mandatori.filter((data) => (data.id !== id)))
            AlertNotification("Berhasil", "Data Mandatori Berhasil Dihapus", "success", 1000);
        } catch(err){
